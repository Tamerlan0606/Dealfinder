...
async function load(){let sr=await fetch('/api/stats');if(!sr.ok)throw new Error('stats HTTP '+sr.status);let s=await sr.json();for(let k of ['buyers','suppliers','hot']){let el=document.getElementById(k);if(el)el.textContent=s[k]??0}
let ar=await fetch('/api/hot');if(!ar.ok)throw new Error('hot HTTP '+ar.status);let a=await ar.json();let el=document.getElementById('list');el.innerHTML=a.length?'':'<div class="card">Пока подходящих закупок нет.</div>';
