...
def stats():
    ...
    hot=c.execute("SELECT count(*) n FROM buyers WHERE fit_status='ЗАХОДИМ'").fetchone()["n"]
    c.close()
    return {"status":"ok","buyers":buyers,"suppliers":sources,"hot":hot}
...
def hot():
    ...
    rows=c.execute("""
        SELECT * FROM buyers
        WHERE fit_status='ЗАХОДИМ'
        ORDER BY COALESCE(advance_pct,0) DESC, COALESCE(budget_rub,0) DESC, id DESC
        LIMIT 50
    """).fetchall()
    result=[]
    for row in rows:
        ...
        raw=d.get("economics_json") or ""
        try:
            d["economics"]=json.loads(raw) if raw else tender_economics(
                d.get("budget_rub"), d.get("advance_pct"),
                security_rub=d.get("security_rub") or 0,
...
async function fetchJson(path, timeoutMs=12000){const ctl=new AbortController();const timer=setTimeout(()=>ctl.abort(),timeoutMs);try{const r=await fetch(path,{cache:'no-store',headers:{'Accept':'application/json'},signal:ctl.signal});const raw=await r.text();let x;try{x=JSON.parse(raw)}catch(_){throw new Error(path+' HTTP '+r.status+': сервер вернул не JSON')}if(!r.ok||x.status==='error')throw new Error(path+' HTTP '+r.status+': '+(x.error||x.status||'ошибка'));return x}finally{clearTimeout(timer)}}
async function load(){const el=document.getElementById('list');try{const d=await fetchJson('/api/dashboard');const st=d.stats||{};for(let k of ['buyers','suppliers','hot']){let n=document.getElementById(k);if(n)n.textContent=st[k]??0}const a=d.hot||[];el.innerHTML=a.length?'':'<div class="card">Пока подходящих закупок нет.</div>';a.forEach(x=>{const e=x.economics||{};el.innerHTML+='<div class="card"><h3>ЗАХОДИМ · '+esc(x.title)+'</h3><div class="muted">'+esc(x.city||'Регион не указан')+' · НМЦК '+Number(x.budget_rub||0).toLocaleString('ru-RU')+' ₽ · аванс '+(x.advance_pct??'—')+'%</div><p>'+esc((x.description||'').slice(0,280))+'</p><div class="money">Маржа: '+Number(e.margin_rub||0).toLocaleString('ru-RU')+' ₽</div><div class="muted">Свои деньги: '+Number(e.own_cash_needed_rub||0).toLocaleString('ru-RU')+' ₽ · цена контракта: '+Number(e.contract_price||0).toLocaleString('ru-RU')+' ₽</div>'+(x.deadline?'<div class="muted">Срок подачи: '+esc(x.deadline)+'</div>':'')+(x.url?'<a class="btn" href="'+esc(x.url)+'" target="_blank">Открыть закупку</a>':'')+'</div>'})}catch(e){el.innerHTML='<div class="card">Ошибка загрузки: '+esc(e.name==='AbortError'?'Таймаут ответа сервера':e.message)+'</div>'}}
