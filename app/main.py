...
async function refresh(){const b=document.querySelector('.refresh');if(b){b.disabled=true;b.textContent='ОБНОВЛЕНИЕ…'}try{const url=new URL('/api/refresh',window.location.href).href;const r=await fetch(url,{method:'GET',cache:'no-store',headers:{'Accept':'application/json'}});const raw=await r.text();let x;try{x=JSON.parse(raw)}catch(_){throw new Error('HTTP '+r.status+': сервер вернул не JSON')}if(!r.ok){throw new Error('HTTP '+r.status+': '+(x.error||x.status||'ошибка сервера'))}if(x.status==='busy'){alert('Обновление уже выполняется. Результаты обновятся автоматически.');await load();return}else if(x.status==='started'){alert('Обновление запущено. Результаты появятся автоматически.');setTimeout(load,3000);return}else if(x.status==='rate_limited'){alert('Источник временно ограничил запросы. Повторить через '+(x.retry_after||300)+' сек.')}else if(x.status!=='ok'&&x.status!=='fallback'){alert('Ошибка источника: '+(x.error||x.warning||('статус '+x.status)+' | raw '+(x.raw??0)));}else if(x.loaded===0){alert('Источник ответил, но 0 закупок прошло фильтры. RAW: '+(x.raw||0)+' | страницы: '+(x.pages||0))}else{alert('Загружено: '+x.loaded+' закупок')}}catch(e){alert('Ошибка соединения: '+(e&&e.message?e.message:String(e)))}finally{if(b){b.disabled=false;b.textContent='ОБНОВИТЬ'}}await load()}load();setInterval(load,60000)
</script></html>'''
